...
- **TWS follow-up missions (design branch `feature/tws-live-advanced-market-data-design`):** parent decision-locking spec approved in `docs/superpowers/specs/2026-06-29-tws-live-advanced-market-data-design.md`. Mission order is (1) **Live trading** — DONE (manual smoke verified 2026-06-30 against a real IB Gateway live account); live place/cancel/modify require explicit allowlisted session arming, backend policy rechecks, and visible ambiguous-outcome handling; arm clears immediately on disconnect; plan archived in `docs/superpowers/plans/2026-06-29-tws-live-trading.md`. (2) **Advanced order types** — IN PROGRESS on `feature/tws-advanced-order-types`; long-only scale-out ladders first, plus brackets, trailing stops, OCA, GTD/MOC/LOC, and conditions; design `docs/superpowers/specs/2026-06-30-tws-advanced-order-types-design.md`, plan `docs/superpowers/plans/2026-07-01-tws-advanced-order-types.md`. Running a Scale-Out Cockpit V2 interlude first (symbol search, scenario calculator, read-only package grouping) before Task 4 brackets; design `docs/superpowers/specs/2026-07-01-scale-out-cockpit-v2-design.md`, plan `docs/superpowers/plans/2026-07-01-scale-out-cockpit-v2.md`. (3) **Market-data extras** — pending; entitlement diagnostics first, then TWS-owned websocket streaming for live quotes/candles, Level 2/depth, and per-exchange guidance; subscription/account-management automation stays out.

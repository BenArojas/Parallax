...
- **TWS Execution Plan Anatomy:** DONE on `feature/tws-execution-plan-anatomy` (2026-07-04) — Execution Plan draft modes now share a UI-only Plan Anatomy surface: Standard explains `WHEN`/`ORDER`, broker effect, price requirement, and review gate; Bracket explains `WHEN`/`ENTER`/`EXIT`/`PROTECT`, cancel rule, and protection state; Scale-Out explains parent entry, per-lot target/protection/fallback isolation, and per-lot cancel semantics; Advanced explains subtype timing/ownership, including the Price Condition `WHEN` gate. The chart remains the only price-line surface; no broker/API behavior changed. Verified with `npm run typecheck`, `git diff --check`, and focused TWS frontend package tests. Design: `docs/superpowers/specs/2026-07-04-tws-execution-plan-anatomy-design.md`; plan: `docs/superpowers/plans/2026-07-04-tws-execution-plan-anatomy.md`.

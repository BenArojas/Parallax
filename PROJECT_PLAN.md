...
- **TWS Execution Plan Anatomy:** DONE on `feature/tws-execution-plan-anatomy` (2026-07-04) — shipped the first UI-only tracer bullet: bracket draft mode now shows Plan Anatomy (`WHEN`/`ENTER`/`EXIT`/`PROTECT` plus broker effect/cancel/protection state) without duplicating the existing chart plan lines. No broker/API behavior changed. Verified with `npm run typecheck` and `git diff --check`. Design: `docs/superpowers/specs/2026-07-04-tws-execution-plan-anatomy-design.md`; plan: `docs/superpowers/plans/2026-07-04-tws-execution-plan-anatomy.md`.

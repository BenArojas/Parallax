...
- **TWS Execution Plan Flow Sheet:** SHIPPED to `dev` from `feature/tws-execution-plan-anatomy` (2026-07-04) — replaced the rejected Plan Anatomy direction with one theme-aware Flow Sheet grammar across Standard, Scale-Out, Bracket, and Advanced; kept mode accents cyan/purple/orange/blue; reduced guidance to one sentence; kept Scale-Out scenario math preview-only with per-lot Target/Stop/EOD assumptions; removed Flow Sheet connection-status duplication and kept chart lines as the only price-line surface. Verified with `npm run typecheck`, `git diff --check`, `npx vitest run src/modules/tws-execution-assistant/scaleOutScenario.test.ts`, browser smoke on `http://localhost:1420/tws` in dark/light across all four modes, `npm run check:policy-drift`, and `npm run build`. Design: `docs/archive/2026-07-04-tws-execution-plan-flow-sheet-design.md`; plan: `docs/archive/2026-07-04-tws-execution-plan-flow-sheet-implementation.md`; mockup: `docs/archive/assets/2026-07-04-execution-plan-flow-sheet-reset.html`.
...
- **TWS cockpit improvements (`feature/tws-*` stacked chain → `dev`, 2026-07-04):** six slices deepening the execution assistant into a position-level decision-support cockpit. (1) **Event-driven reconciliation** — the adapter now subscribes `orderStatusEvent`/`execDetailsEvent`/`positionEvent`/`openOrderEvent` and pushes a debounced `tws_recon_changed` WS event; the frontend refetches on receipt, so the recon poll dropped to a 30s fallback and the manual "Refresh Open Orders" button is gone. (2) **Trading-focused positions table** — dropped ConID from display (still the internal key); added Mkt Price, Daily P&L, Unrealized P&L ($ and %), and an OCA-and-held-children-aware Protection column, all enriched from `ib.portfolio()` + lazy per-position `reqPnLSingle` subscriptions synced to held positions; row click repoints the chart with a read-only avg-cost line. (3) **Account-level Day P&L strip** via a single `reqPnL` subscription. (4) **Manage Position panel** — a new **flatten order path** (`flatten-paper`/`flatten-live` endpoints → `flatten_position()`): cancels ALL working orders on the conid, awaits each cancel confirmation with a shared 5s deadline (typed `flatten_cancel_timeout`, no close placed on timeout), settle-beat + re-reads the position, then market-closes the *actual* remaining quantity so a raced fill can't double-size; partial sell/double actions only PREFILL the standard ticket (no direct submission) and never touch package-covered shares. (5) **Fill toasts** — `tws_fill` from `execDetailsEvent`, suppressed during the connect-time execution replay, with scale-out/bracket/flatten role suffixes. (6) **Frontend risk-based sizing** (`Risk $` → `floor(risk/|entry−stop|)`) on the standard and bracket forms, kept in sync when prices change. Reviewed at two adversarial multi-agent checkpoints; findings fixed (DBL_MAX P&L sentinel, protection double-count and held-bracket-children, review-lock on row click, per-account `reqPnLSingle`, risk-sizing staleness, flatten order-ref mis-flagged as an unknown package role). Broker mutations stay behind `TwsBrokerAdapter`; no DB persistence, no autonomous trading; flatten reuses the same paper/live policy gates as `place_order`. Verified: `npm run typecheck`, backend `py_compile`, `npm run check:policy-drift`, and user paper-TWS smoke (flatten/partial/prefill/positions table/risk sizing green; fill-toast + real-time-update checks pending live market data). Plan: `docs/superpowers/plans/2026-07-04-tws-cockpit-improvements-plan.md`.
